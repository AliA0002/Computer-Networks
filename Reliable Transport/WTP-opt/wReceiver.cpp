...
void dump_data(shared_ptr<Packet> packet, args_t &args, int connection) {
  ...
  // std::cout << "Writing chunk: " << packet->getHeader().seqNum << endl;
  char buf[MTU];
  ...
  while (true) {
    ...
    if (maybe_data->getHeader().seqNum <
      ...
      if (maybe_data->getHeader().seqNum == first_false(receivedChunks)) {
        ...
        buffer.push_back(maybe_data);
        // receivedChunks[maybe_data->getHeader().seqNum] = true;
      }
      for (auto i = buffer.begin(); i != buffer.end(); i++) {
        if ((*i)->getHeader().seqNum == first_false(receivedChunks)) {
          dump_data(*i, args, conn_count);
          receivedChunks[(*i)->getHeader().seqNum] = true;
          //   i = buffer.erase(i);
          i = buffer.begin();
        }
      }
    ...
    {
      Packet ack(PACKET_ACK, maybe_data->getHeader().seqNum);
      if (ack.send(fd, &sender))
        args.log->write(ack.getHeader());
      ...
      while (!recvd_end) {
        ...
        if (next->validate() && next->getHeader().type == PACKET_DATA) {
          // std::cout << "Got Data " << next->getHeader().seqNum << endl;
          if (!receivedChunks[next->getHeader().seqNum] &&
            ...
            Packet ack(PACKET_ACK, next->getHeader().seqNum);
            if (ack.send(fd, &sender))
              args.log->write(ack.getHeader());
            // std::cout << "Acking " << ack.getHeader().seqNum << endl;
            // else = duplicate or out of range, but ack it anyway
          } else if (receivedChunks[next->getHeader().seqNum]) {
            Packet ack(PACKET_ACK, next->getHeader().seqNum);
            if (ack.send(fd, &sender))
              args.log->write(ack.getHeader());
            // std::cout << "Acking " << ack.getHeader().seqNum << endl;
