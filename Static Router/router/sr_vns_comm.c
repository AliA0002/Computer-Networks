...
static int sr_arp_req_not_for_us(struct sr_instance *sr, uint8_t *packet /* lent */, unsigned int len, char *interface /* lent */);
...
int sr_read_from_server_expect(struct sr_instance *sr /* borrowed */, int expected_cmd) {
    ...
    *((int *)buf) = htonl(len);
    ...
    case VNSPACKET:
        ...
        if (sr_arp_req_not_for_us(sr, (buf + sizeof(c_packet_header)), len - sizeof(c_packet_ethernet_header) + sizeof(struct sr_ethernet_hdr),
        ...
        sr_handlepacket(sr, (buf + sizeof(c_packet_header)), len - sizeof(c_packet_ethernet_header) + sizeof(struct sr_ethernet_hdr),
                        (char *)(buf + sizeof(c_base)));
...
int sr_arp_req_not_for_us(struct sr_instance *sr, uint8_t *packet /* lent */, unsigned int len, char *interface /* lent */) {
    ...
    if ((e_hdr->ether_type == htons(ethertype_arp)) && (a_hdr->ar_op == htons(arp_op_request)) && (a_hdr->ar_tip != iface->ip)) {
        return 1;
    }
